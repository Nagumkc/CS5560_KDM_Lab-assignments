...
object SparkLDAMain {
  ...
  private def run(params: Params) {
    ...
    topics.take(5).zipWithIndex.foreach { case (topic, i) =>
      println(s"TOPIC $i")
      // topic_output.println(s"TOPIC $i")
      topic.take(5).foreach { case (term, weight) =>
...
*/
    ...
    val tf = hashingTF.transform(df).zipWithIndex().map(_.swap)
    tf.cache()
   ...
   val dff= df.flatMap(f=>f)
    ...
    (tf,vocab, dff.count()) // Vector, Vocab, total token count
  }
}
